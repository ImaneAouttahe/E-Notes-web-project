...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">creer emploi niveau</h3>
</div>
<div class="col-md-6">
...
<?php if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['niveau_id'])):
        $selected_niveau = null;
        foreach ($niveaux as $niveau) {
            if ($niveau['id_n'] == $_POST['niveau_id']) {
                $selected_niveau = $niveau;
                break;
            }
        }
        if ($selected_niveau) {
            $display_name = $selected_niveau['nom_n'] . ' S' . $selected_niveau['id_s'];
            if ($selected_niveau['id_op'] != 0) {
                $stmt_option = $pdo->prepare('SELECT nomop FROM choix_3 WHERE idop = :id_op');
                $stmt_option->execute(['id_op' => $selected_niveau['id_op']]);
    ...
    ?>
<form action="save_emploi.php" method="post">
<table>
            <tr>
                <th colspan="8">
                    <?= "Niveau: "  . htmlspecialchars($display_name); ?>
                </th>
            </tr>
    <!-- En-têtes du tableau -->
    <tr>
...
<?php 
// Assuming you have database connection setup as $pdo
$host = 'localhost';
$username = 'root';
$password = '';
$dbname = 'enotee';
...
try {
    if (isset($_POST['niveau_id'])) {
        ...
        // Sélection des modules en fonction de l'identifiant du niveau
        $stmt = $pdo->prepare("SELECT m.id_mod, m.nom_mod 
                               FROM modules m
                               JOIN niveau mn ON m.id_n = mn.id_n
                               WHERE mn.id_n = ?");
        $stmt->execute([$niveau_id]);

        $modules = $stmt->fetchAll(PDO::FETCH_ASSOC);
        echo "<h3>Modules :</h3><ul id='modulesList' style='list-style: none;'>"; // Supprime les puces de liste par défaut
        $colorClasses = ['blue', 'yellow', 'purple']; // Les noms des classes correspondent au CSS ci-dessus
        $index = 0; // Index pour parcourir les classes de couleur
        
        foreach ($modules as $module) {
            $colorClass = $colorClasses[$index % count($colorClasses)]; // Choix cyclique de la classe de couleur
            echo "<li id='module-" . htmlspecialchars($module['id_mod']) . "' draggable='true' class='module " . $colorClass . "'>" . htmlspecialchars($module['nom_mod']) . "</li>";
            $index++; // Incrémenter l'index pour le prochain module
        }
...
document.getElementById('btnEnregistrer').addEventListener('click', function() {
    generatePDF(function(pdf, filename) {
        ...
        xhr.open('POST', 'savee_pdf.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.send('pdfBase64=' + encodeURIComponent(pdfBase64) + '&filename=' + encodeURIComponent(filename) + '&niveau_id=' + encodeURIComponent(<?php echo json_encode($niveau_id); ?>));
    });
});
