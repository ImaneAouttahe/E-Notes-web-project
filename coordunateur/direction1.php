...
// Récupérez l'ID de la filière sélectionnée depuis le formulaire
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['filiere_id'])) {
    $filiere_id = $_POST['filiere_id'];

    // Sélectionnez les étudiants de la filière choisie avec leur niveau
    $stmt_etudiants = $pdo->prepare('SELECT u.nom, u.prenom, u.email,u.img, n.nom_n
                                    FROM utilisateurs u
                                    INNER JOIN etudiants e ON u.id_ut = e.id_ut
                                    INNER JOIN niveau n ON e.id_n = n.id_n
                                    WHERE e.id_f = :filiere_id');
    $stmt_etudiants->execute(['filiere_id' => $filiere_id]);
    $etudiants = $stmt_etudiants->fetchAll(PDO::FETCH_ASSOC);
} else {
    // Rediriger l'utilisateur vers la page de choix de filière si aucune filière n'est sélectionnée
...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">etduiants filiere</h3>
</div>
<div class="col-md-6">
            ...
            <table class="custom-table">
                ...
                <tbody>
                    <?php foreach ($etudiants as $etudiant): ?>
                        <tr>
                            <td><img  src="../les_images/<?= $etudiant['img'] ?>" width="30" alt="etudiant"></td>  
                            <td><?= $etudiant['nom'] ?></td>
                            <td><?= $etudiant['prenom'] ?></td>
                            <td><?= $etudiant['email'] ?></td>
                            <td><?= $etudiant['nom_n'] ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
