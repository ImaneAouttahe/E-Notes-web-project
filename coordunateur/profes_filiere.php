...
try {
    ...
    $email = $_SESSION['email'];
    $password = $_SESSION['password'];
    $stmt_filiere_id = $pdo_enote->prepare('SELECT filieres.id_f
                                        FROM utilisateurs 
                                        INNER JOIN cordonnateur ON utilisateurs.id_ut = cordonnateur.id_ut 
                                        INNER JOIN filieres ON cordonnateur.id_f = filieres.id_f
                                        WHERE utilisateurs.EMAIL = :email AND utilisateurs.MDP = :password');


// Exécutez la requête avec les valeurs fournies
$stmt_filiere_id->execute(['email' => $email, 'password' => $password]);

// Récupérez l'ID de la filière
$filiere_id = $stmt_filiere_id->fetchColumn();
$stmt_id = $pdo_enote->prepare('SELECT utilisateurs.id_ut, utilisateurs.nom, utilisateurs.prenom, utilisateurs.email ,utilisateurs.img ,profes.specialite , modules.nom_mod
FROM modules
INNER JOIN profes ON modules.id_prof = profes.id_p 
INNER JOIN utilisateurs ON utilisateurs.id_ut = profes.id_ut
WHERE modules.id_fil = :filiere_id');

// Exécutez la requête avec l'ID de la filière
$stmt_id->execute(['filiere_id' => $filiere_id]);
...
<li >
<a href="#"><img src="assets/img/sidebar/icon-4.png" alt="icon"> <span> profes filiere</span></a>
</li>
<li >
...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">etudiants filiere</h3>
</div>
<div class="col-md-6">
...
<table class="custom-table">
    ...
    <tbody>
        <?php
        if ($result) {
            $professeurs_modules = array();

            // Collecte des modules affectés de chaque professeur
            do {
                $prof_id = $result['id_ut'];
                if (!isset($professeurs_modules[$prof_id])) {
                    $professeurs_modules[$prof_id] = array(
                        'img' => $result['img'],
                        'nom' => $result['nom'],
                ...
                $professeurs_modules[$prof_id]['modules'][] = $result['nom_mod'];
            } while ($result = $stmt_id->fetch());

            // Affichage des professeurs avec leurs modules affectés
            foreach ($professeurs_modules as $prof) {
                echo "<tr>";
                echo '<td><img src="../les_images/' . $prof['img'] . '" width="30" alt="etudiant"></td>';
                echo "<td>" . $prof['nom'] . "</td>";
                echo "<td>" . $prof['prenom'] . "</td>";
                echo "<td>" . $prof['specialite'] . "</td>";
                echo "<td>" . implode(", ", $prof['modules']) . "</td>"; // Affiche tous les modules du professeur séparés par une virgule
                echo "</tr>";
            }
        } else {
