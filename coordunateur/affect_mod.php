...
// Vérification de l'existence des informations de session
if (!isset($_SESSION['email']) || !isset($_SESSION['password'])) {
    exit("Session expirée ou non authentifiée");
}
...
// Récupération de l'ID du département de l'utilisateur connecté
$stmt_departement_id = $pdo->prepare('SELECT id_d FROM filieres WHERE id_f IN (SELECT id_f FROM cordonnateur WHERE id_ut IN (SELECT id_ut FROM utilisateurs WHERE email = :email AND mdp = :password))');
$stmt_departement_id->execute(['email' => $_SESSION['email'], 'password' => $_SESSION['password']]);
$departement_id = $stmt_departement_id->fetchColumn();

// Sélection de tous les professeurs associés à ce département avec leurs noms et prénoms
$stmt_professeurs = $pdo->prepare('SELECT p.*, u.nom, u.prenom FROM profes p INNER JOIN utilisateurs u ON p.id_ut = u.id_ut WHERE p.id_d = :departement_id');
$stmt_professeurs->execute(['departement_id' => $departement_id]);
$professeurs = $stmt_professeurs->fetchAll(PDO::FETCH_ASSOC);

$stmt_departement_id = $pdo->prepare('SELECT id_f FROM cordonnateur WHERE id_ut IN (SELECT id_ut FROM utilisateurs WHERE email = :email AND mdp = :password)');
$stmt_departement_id->execute(['email' => $_SESSION['email'], 'password' => $_SESSION['password']]);
$filiere_id = $stmt_departement_id->fetchColumn();

// Sélection de tous les modules associés à ce département
$stmt_modules = $pdo->prepare('SELECT * FROM modules WHERE id_fil = :filiere_id');
$stmt_modules->execute(['filiere_id' => $filiere_id]);
$modules = $stmt_modules->fetchAll(PDO::FETCH_ASSOC);

$stmt_affectations = $pdo->prepare('SELECT id_mod FROM modules WHERE id_prof = :id_prof');
$affectations = array();
foreach ($professeurs as $professeur) {
    $stmt_affectations->execute(['id_prof' => $professeur['id_p']]);
    $affectations[$professeur['id_p']] = $stmt_affectations->fetchAll(PDO::FETCH_COLUMN);
}

// Traitement du formulaire soumis pour affecter les modules aux professeurs
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['affectations'])) {
    // Début de la transaction
    $pdo->beginTransaction();

    try {
        foreach ($_POST['affectations'] as $id_prof => $module_ids) {
            // Récupérer les modules déjà affectés à ce professeur
            $stmt_current_affectations = $pdo->prepare('SELECT id_mod FROM modules WHERE id_prof = :id_prof');
            $stmt_current_affectations->execute(['id_prof' => $id_prof]);
            $current_affectations = $stmt_current_affectations->fetchAll(PDO::FETCH_COLUMN);

            // Décocher les modules non sélectionnés dans le formulaire
            $modules_to_unset = array_diff($current_affectations, $module_ids);
            foreach ($modules_to_unset as $module_id_to_unset) {
                $stmt_unset_module = $pdo->prepare('UPDATE modules SET id_prof = 0 WHERE id_mod = :module_id');
                $stmt_unset_module->execute(['module_id' => $module_id_to_unset]);
            }

            // Cocher les nouveaux modules sélectionnés dans le formulaire
            $modules_to_set = array_diff($module_ids, $current_affectations);
            foreach ($modules_to_set as $module_id_to_set) {
                $stmt_set_module = $pdo->prepare('UPDATE modules SET id_prof = :id_prof WHERE id_mod = :module_id');
                $stmt_set_module->execute(['id_prof' => $id_prof, 'module_id' => $module_id_to_set]);
            }
        }
...
<li>
<a href="#"><img src="assets/img/sidebar/icon-14.png" alt="icon"> <span>Affectation modules</span></a>
</li>
<li>
...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">affectation modules </h3>
</div>
<div class="col-md-6">
                ...
                <td>
                    <details>
                        <div class="modules-container">
                            ...
                            <?php
                            $checked = in_array($module['id_mod'], $affectations[$professeur['id_p']]) ? 'checked' : '';
                            ?>
                            <label>
                                <input type="checkbox" name="affectations[<?= $professeur['id_p'] ?>][]" value="<?= $module['id_mod'] ?>" <?= $checked ?>>
                                <?= $module['nom_mod'] ?>
                                <br>
                            </label>
                            <?php endforeach; ?>
