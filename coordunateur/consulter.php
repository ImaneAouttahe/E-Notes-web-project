...
<style>
    ...
    .btn-light-green:hover {
        background-color: #77dd77; /* Darker shade of light green */
        color: white;
    }
    .btn-light-red {
        background-color: #f08080; /* Light red color */
        color: white;
    }
    .btn-light-red:hover {
        background-color: #ee5c5c; /* Darker shade of light red */
        color: white;
    }
...
<li><a href="#"><span>consulter emploi </span></a></li>
</ul>
</li>
<li>
...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">consulter emplois profes</h3>
</div>
<div class="col-md-6">
...
try {
    ...
    SELECT 
        fichiers_pdf.id_ut,
        fichiers_pdf.nom_fichier,
        fichiers_pdf.contenu_pdf,
        fichiers_pdf.date_creation,
        utilisateurs.nom,
        utilisateurs.prenom
    FROM 
        fichiers_pdf
    JOIN 
        utilisateurs ON fichiers_pdf.id_ut = utilisateurs.id_ut;
    ";
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        // Assuming the files are stored in an 'emploi' directory
        $fileUrl = './emploi/' . htmlspecialchars($row['nom_fichier']);
        echo "<tr>
                <td>" . htmlspecialchars($row['nom']) . "</td>
                <td>" . htmlspecialchars($row['prenom']) . "</td>
                <td><a href='" . $fileUrl . "'>" . htmlspecialchars($row['nom_fichier']) . "</a></td>
                <td>" . htmlspecialchars($row['date_creation']) . "</td>
                <td>
                    <a href='edit.php?filename=" . urlencode($row['nom_fichier']) . "' class='btn btn-light-green btn-sm mb-1'>
                        <i class='far fa-edit'></i>
                    </a>
                    <button type='button' data-toggle='modal' data-target='#delete_employee' class='btn btn-light-red btn-sm mb-1'>
