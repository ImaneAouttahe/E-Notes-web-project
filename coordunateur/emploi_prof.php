...
<li><a href="#"><span>creer emploi prof </span></a></li>
...
<form id="myForm" action="direction10.php" method="POST">
    ...
    <select name="professeur_id" class="form-control">
        ...
        try {   
            $stmt_id_filiere = $pdo->prepare("SELECT c.id_f FROM cordonnateur c join utilisateurs ON c.id_ut = utilisateurs.id_ut where email = :email AND mdp = :password ");
            $stmt_id_filiere->execute(array('email' => $email, 'password' => $password));
            $id_filiere = $stmt_id_filiere->fetchColumn();

            if (!$id_filiere) {
                throw new Exception("Aucune filière associée au cordonnateur.");
            }

            $sql = "SELECT DISTINCT u.nom, u.prenom, u.id_ut
                    FROM modules m
                    JOIN profes p ON m.id_prof = p.id_p
                    JOIN utilisateurs u ON p.id_ut = u.id_ut
                    WHERE m.id_fil = :id_f";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':id_f', $id_filiere, PDO::PARAM_INT);
            $stmt->execute();
            $professeurs = $stmt->fetchAll(PDO::FETCH_ASSOC);

            foreach ($professeurs as $professeur) {
                echo '<option value="'. htmlspecialchars($professeur['id_ut']) .'">' . htmlspecialchars($professeur['nom']) . " " . htmlspecialchars($professeur['prenom']) . '</option>';
            }
        } catch (PDOException $e) {
    ...
    <button type="submit" class="choix">Choisir</button>
</form>
