...
$stmt_prof_id = $pdo->prepare('SELECT id_f FROM cordonnateur WHERE id_ut IN (SELECT id_ut FROM utilisateurs WHERE email = :email AND mdp = :password)');
$stmt_prof_id->execute(['email' => $_SESSION['email'], 'password' => $_SESSION['password']]);
$filiere_id = $stmt_prof_id->fetchColumn();

// Sélectionnez tous les modules affectés à ce professeur
$stmt_modules = $pdo->prepare('SELECT DISTINCT m.id_mod, m.nom_mod
                                FROM modules m
                                WHERE m.id_fil = :filiere_id');
$stmt_modules->execute(['filiere_id' => $filiere_id]);
$modules = $stmt_modules->fetchAll(PDO::FETCH_ASSOC);

$stmt_niveaux = $pdo->prepare('SELECT DISTINCT n.nom_n
                                FROM niv_fil nf
                                JOIN niveau n ON nf.id_n=n.id_n
                                WHERE nf.id_f = :filiere_id GROUP BY nom_n');
$stmt_niveaux->execute(['filiere_id' => $filiere_id]);
...
<ul class="list-unstyled" style="display: none;">
<li><a href="#"><span>Notes modules</span></a></li>
...
<div class="form-container container-fluid">
    <form method="post" action="direction7.php">
        <div class="row">
            <div class="col-md-12">
                <h3> Niveau : </h3>
                <select name="niveau_id" id="niveau_id" class="form-control">
                    ...
                    <?php foreach ($niveaux as $niveau): ?>
                        <option value="<?= $niveau['nom_n'] ?>"><?= $niveau['nom_n'] ?></option>
                    <?php endforeach; ?>
                </select>
                <br>
            </div>
