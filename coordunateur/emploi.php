...
<li><a href="#"><span>creer emploi niveau</span></a></li>
...
<form id="myForm" action="direction8.php" method="POST">
    ...
    <select name="niveau_id" class="form-control">
        ...
        <?php foreach ($niveaux as $niveau): ?>
            <?php
            // Vérifier si l'id_op est différent de 0 et récupérer le nom de l'option
            $display_name = $niveau['nom_n'] . ' S' . $niveau['id_s']; // Nom de base avec semestre
            if ($niveau['id_op'] != 0) {
                ...
                if ($nom_option) {
                    $display_name .= ' option : ' . $nom_option; // Ajouter l'option si disponible
                }
            }
            ?>
            <option value="<?= htmlspecialchars($niveau['id_n']); ?>">
