...
// Vérifier si les identifiants de filière et de niveau sont définis dans la requête POST
if (!isset($_POST['filiere_id']) || !isset($_POST['niveau_id'])) {
    exit('Veuillez sélectionner une filière et un niveau.');
}

// Récupérer les identifiants de filière et de niveau depuis la requête POST
...
// Récupérer les données des étudiants en fonction de la filière et du niveau choisis
$sql_etudiants = $pdo->prepare('SELECT u.nom, u.prenom, u.ville, u.email, u.img , n.nom_n , f.nom_f
                                FROM utilisateurs u 
                                INNER JOIN etudiants e ON u.id_ut = e.id_ut 
                                JOIN filieres f ON e.id_f= f.id_f
                                JOIN niveau n ON e.id_n = n.id_n
                                WHERE e.id_f = :filiere_id AND e.id_n = :niveau_id');
$sql_etudiants->execute(['filiere_id' => $filiere_id, 'niveau_id' => $niveau_id]);
$etudiants = $sql_etudiants->fetchAll(PDO::FETCH_ASSOC);
?>
...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">consulter etudiants</h3>
</div>
<div class="col-md-6">
    ...
    <div class="content container-fluid">
        <div class="row">
            ...
            <div class="alert alert-info" role="alert" id="el1">
                            filiere: <?= $etudiants[0]['nom_f'] ?>&nbsp;&nbsp;&nbsp;&nbsp;     
                            niveau: <?= $etudiants[0]['nom_n'] ?>
                        </div>
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><img src="../les_images/<?= $etudiant['img'] ?>" alt="Image de l'étudiant" width="50"></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
