...
try {
    ...
    $stmt_id = $pdo_enote->prepare('SELECT utilisateurs.id_ut, chef_d.id_d, departements.nom_d,
                                        filieres.nom_f, modules.nom_mod
                                    FROM utilisateurs 
                                    INNER JOIN chef_d ON utilisateurs.id_ut = chef_d.id_ut 
                                    INNER JOIN departements ON chef_d.id_d = departements.id_d 
                                    INNER JOIN filieres ON departements.id_d = filieres.id_d
                                    INNER JOIN modules ON filieres.id_f = modules.id_fil
                                    WHERE utilisateurs.EMAIL = :email AND utilisateurs.MDP = :password');

    // Exécuter la requête avec les valeurs fournies
    $stmt_id->execute(['email' => $email, 'password' => $password]);
    ...
    if ($result) {
        $id_departement = $result['id_d'];
        $nom_departement = $result['nom_d'];
        $nom_filiere = $result['nom_f'];
        $nom_module = $result['nom_mod'];
    } else {
...
<li>
<a href="#"><img src="assets/img/sidebar/icon-5.png" alt="icon"> <span>Modules dep</span></a>
</li>
<li>
...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">modules de departement</h3>
</div>
<div class="col-md-6">
...
<table class="custom-table">
        ...
        <tbody>
            ...
            if ($result) {
                do {
                    echo "<tr>";
                    echo "<td>" . $result['nom_mod'] . "</td>"; // Affiche le nom du module
                    echo "<td>" . $result['nom_f'] . "</td>"; // Affiche le nom de la filière
                    echo "</tr>";
