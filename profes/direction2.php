...
$type_epreuve = filter_input(INPUT_POST, 'type_epreuve', FILTER_SANITIZE_STRING);
$pourcentage = filter_input(INPUT_POST, 'pourcentage', FILTER_VALIDATE_FLOAT);

// Vérifiez si toutes les données nécessaires sont présentes
if ($niveau_id && $module_id && $type_epreuve && $pourcentage !== false) {
    // Préparez et exécutez la requête pour récupérer les étudiants
    $stmt_etudiants = $pdo->prepare('SELECT u.id_ut, u.nom, u.prenom, u.email, n.nom_n, e.id_et
    FROM utilisateurs u
    INNER JOIN etudiants e ON u.id_ut = e.id_ut
    INNER JOIN niveau n ON e.id_n = n.id_n
    WHERE e.id_n = :niveau_id');
    $stmt_etudiants->bindValue(':niveau_id', $niveau_id, PDO::PARAM_INT);
    $stmt_etudiants->execute();
    $etudiants = $stmt_etudiants->fetchAll(PDO::FETCH_ASSOC);

    // Préparez et exécutez la requête pour récupérer les notes existantes
    $stmt_notes = $pdo->prepare('SELECT n.id_et, n.note, u.id_ut
    FROM notes n
    INNER JOIN etudiants e ON n.id_et = e.id_et
    INNER JOIN utilisateurs u ON e.id_ut = u.id_ut
    WHERE n.id_n = :niveau_id AND n.id_m = :module_id AND n.type_epreuve = :type_epreuve AND n.pourcentage = :pourcentage');
$stmt_notes->bindValue(':niveau_id', $niveau_id, PDO::PARAM_INT);
$stmt_notes->bindValue(':module_id', $module_id, PDO::PARAM_INT);
$stmt_notes->bindValue(':type_epreuve', $type_epreuve, PDO::PARAM_STR);
$stmt_notes->bindValue(':pourcentage', $pourcentage, PDO::PARAM_INT);
$stmt_notes->execute();
            ...
            <?php foreach ($etudiants as $etudiant): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hidden" name="id_ut[]" value="<?= $etudiant['id_ut'] ?>">
                        <?php
                            // Vérifiez si une note existe pour cet étudiant
                            $note_value = isset($notes[$etudiant['id_et']]) ? $notes[$etudiant['id_et']] : '';
                            echo '<input type="text" name="note[]" class="form-control" placeholder="Entrez la note" value="' . $note_value . '">';
                        ?>
                    </td>
                </tr>
            <?php endforeach; ?>
...
<?php
    ...
    header('Location: direction2.php');
    exit();
}
?>